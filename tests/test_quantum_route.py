...
def test_trq009_executes_key_presses_and_mouse_movement_in_order():
    ...
    with patch("ferdi.main.pydirectinput.press", mock_press), \
    ...
    assert any("F2" in str(call) or "f2" in str(call).lower() for call in first_calls), \
        "key_open (F2) should be pressed"

    # Verify moveTo was called to move mouse to search field
    assert mock_move.call_count >= 1, "pydirectinput.moveTo should be called to position mouse"
    ...
    assert mock_typewrite.call_count >= 1, "pydirectinput.typewrite should be called to type destination"

    # Verify key_validate (enter) was pressed
    enter_pressed = any("enter" in str(call).lower() or "return" in str(call).lower()
    ...
    assert mock_press.call_count >= 3, "Multiple key presses expected (open, validate, close, quantum)"
...
def test_trq010_route_validator_interface_exists():
    ...
    assert hasattr(RouteValidator, "validate"), "RouteValidator must have validate method"
    ...
    for call in calls:
        if len(call[0]) >= 2:
            ...
            if x == 640 and y == 144:
                ...
                break
    assert found_correct_coordinates, f"Expected moveTo(640, 144) to be called, but got: {calls}"
