...
Acceptance criteria verified locally (file existence + YAML content):
  ...
  AC4: Python 3.11 is set up via astral-sh/setup-uv
  AC5: Dependencies installed via uv sync --dev
  AC6: Test suite executed with uv run pytest tests/ -v
    ...
    for job in jobs.values():
        for step in job.get("steps", []):
            ...
            if uses.startswith("astral-sh/setup-uv"):
                found = True
                break
    assert found, "No step uses 'astral-sh/setup-uv'."


def test_trq002_python_version_is_311(workflow_yaml):
    """AC4b: The setup-uv step must configure python-version '3.11'."""
    jobs = workflow_yaml.get("jobs", {})
    python_version = None
    for job in jobs.values():
        for step in job.get("steps", []):
...
def test_trq002_pytest_command(workflow_yaml):
    ...
    for job in jobs.values():
        for step in job.get("steps", []):
            run_cmd = step.get("run", "")
            if "uv run pytest tests/ -v" in run_cmd:
                found = True
                break
    assert found, (
        "No step contains 'uv run pytest tests/ -v'. "
        "The test suite must be executed via uv with verbose output."
    )
